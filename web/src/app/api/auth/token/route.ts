...
export async function POST(req: NextRequest) {
    ...
    const be = await fetch(`${BACKEND_URL}/auth/token`, {
        method: "POST",
    ...
    const res = new NextResponse(text, {
        status: be.status,
        headers: { "Content-Type": be.headers.get("content-type") ?? "application/json" },
    })
...
export async function GET(req: NextRequest) {
    ...
    const beSetCookie = be.headers.get("set-cookie")
    if (beSetCookie) res.headers.set("set-cookie", beSetCookie)
