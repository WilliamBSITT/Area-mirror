...
type UseServiceActionParams = {
    data: ServicesActionsParams | null;
    loading: boolean;
    error: Error | null;
...
export function useServiceActionParams(
    ...
    const [data, setData] = useState<ServicesActionsParams | null>(null);
    ...
    const fetchParams = useCallback(async () => {
        ...
        try {
            const res = await fetch(
                `/api/services/${encodeURIComponent(serviceName)}/actions/${encodeURIComponent(actionName)}/params`,
                { signal: controller.signal }
            );
            if (!res.ok) throw new Error(`Request failed with ${res.status}`);
            const json = (await res.json()) as ServicesActionsParams;
            setData(json);
        } catch (e: any) {
